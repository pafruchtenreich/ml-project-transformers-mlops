...
class RNN(nn.Module):
    def __init__(self, input_size: int, hidden_size: int, output_size: int, num_layers: int):
        ...
        self.h2h_layers = nn.ModuleList([nn.Linear(hidden_size, hidden_size) for _ in range(num_layers - 1)])
    ...
    def forward(self, x: torch.Tensor, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        for layer_index, h2h in enumerate(self.h2h_layers, start=1):
            hidden_states[layer_index] = torch.tanh(h2h(hidden_states[layer_index - 1]) + hidden_states[layer_index])
